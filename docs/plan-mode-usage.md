...
- 单独批准某个步骤或只批准低风险步骤。
- 多个写入步骤在独立 worktree 中自动并行。
- 自动合并 Implementer Patch。
- 完整成本估算和费用预算。
- 应用退出后由 daemon 持续执行。
...
步骤依赖必须构成无环图。当前执行器一次只推进一个正式写入步骤，但只读 Worker 可以在步骤内部并行调查。
...
7. Task Center 显示当前步骤、Run、审批、Worker 和 Artifact。
...
当前同一 Plan revision 只允许一个正式步骤处于 `running`。步骤关联的只读 Worker 可以并行运行。

## 暂停、恢复和重试

Plan 面板和 Task Center 都可以控制执行 Task：

- “暂停执行”：停止当前 Run，并保存可恢复状态。
- “恢复执行”：从 `paused` 或 `interrupted` 创建新的 Run。
- “重试执行”：对 `failed` Task 创建新的 attempt。
- “打开任务”：查看审批、事件、Run 和 Artifact。
- “打开会话”：回到关联聊天上下文。
...
Plan 的规划阶段和执行阶段都可以进入后台：

- 在“规划”模式点击 `↗`：后台生成 Plan。
- 批准后从 Plan 面板打开执行 Task：在 Task Center 跟踪。
- 执行中可以暂停、恢复或处理审批。
...
详细生命周期见[后台任务使用指南](./background-tasks-usage.md)。
...
只有最新 Plan 状态为 `ready`，并且对应规划 Task 已成功完成时才能批准。如果仍显示“正在完成规划”，请查看 Task Center 是否在等待审批、等待输入、暂停或失败。
...
可以。在“规划”模式点击 `↗`。会话命令不能后台运行，但普通规划 Prompt 可以。
