...
function MemorySettings({ value, source, zh, update, hasWorkspace, taskId }: SettingsComponentProps & { hasWorkspace: boolean; taskId?: string }) {
  ...
  const orderedMemories = [...memories].sort((left, right) => {
    const category = (memory: MemoryRecord) => memory.status === "pending"
      ? 2
      : memory.source.kind === "session_handoff" || memory.type === "task-state"
        ? 1
        : 0;
  ...
  return <>
    ...
    <Toggle title={zh ? "自动沉淀项目记忆" : "Automatic project memory"} description={zh ? "成功对话后自动更新可信知识和分支交接；不确定内容仍需确认。" : "Updates verified knowledge and branch handoffs after successful turns; uncertain items still require review."} path="memory.automaticCandidates" checked={value.memory.automaticCandidates} source={source} onChange={(automaticCandidates) => update({ memory: { automaticCandidates } })} />
    <Toggle title={zh ? "自动接受有证据的记忆" : "Auto-accept evidenced memories"} description={zh ? "仅接受引用用户消息或成功 Tool 结果且达到置信阈值的内容。" : "Only accepts items citing a user message or successful Tool result above the confidence threshold."} path="memory.automaticAcceptVerified" checked={value.memory.automaticAcceptVerified} source={source} onChange={(automaticAcceptVerified) => update({ memory: { automaticAcceptVerified } })} />
    <Range title={zh ? "自动接受置信度（百分比）" : "Auto-accept confidence (%)"} path="memory.automaticAcceptanceThreshold" value={Math.round(value.memory.automaticAcceptanceThreshold * 100)} min={50} max={100} step={1} source={source} onChange={(automaticAcceptanceThreshold) => update({ memory: { automaticAcceptanceThreshold: automaticAcceptanceThreshold / 100 } })} />
    <Range title={zh ? "交接摘要保留天数" : "Handoff retention days"} path="memory.handoffRetentionDays" value={value.memory.handoffRetentionDays} min={1} max={90} source={source} onChange={(handoffRetentionDays) => update({ memory: { handoffRetentionDays } })} />
    <Range title={zh ? "每轮交接摘要数量" : "Handoffs per turn"} path="memory.handoffRecallLimit" value={value.memory.handoffRecallLimit} min={0} max={10} source={source} onChange={(handoffRecallLimit) => update({ memory: { handoffRecallLimit } })} />
    <Range title={zh ? "交接摘要 Token 预算" : "Handoff token budget"} path="memory.handoffTokenBudget" value={value.memory.handoffTokenBudget} min={0} max={10000} step={50} source={source} onChange={(handoffTokenBudget) => update({ memory: { handoffTokenBudget } })} />
