...
const agentSchema = z.object({
  ...
  workerMaxPerRoot: z.number().int().min(1).max(4),
  workerTimeoutMs: z.number().int().min(10_000).max(3_600_000),
  workerMaxInputTokens: z.number().int().min(1_000).max(10_000_000),
  workerMaxOutputTokens: z.number().int().min(256).max(1_000_000),
  workerMaxToolCalls: z.number().int().min(1).max(1_000),
  workerModel: z.string(),
  dagExecutionEnabled: z.boolean(),
  planMaxConcurrentSteps: z.number().int().min(1).max(8),
  planMaxDurationMs: z.number().int().min(10_000).max(86_400_000),
  planMaxInputTokens: z.number().int().min(1_000).max(100_000_000),
  planMaxOutputTokens: z.number().int().min(256).max(10_000_000),
  planMaxToolCalls: z.number().int().min(1).max(100_000),
...
const memorySchema = z.object({
  ...
  automaticAcceptanceThreshold: z.number().min(0.5).max(1),
  handoffRetentionDays: z.number().int().min(1).max(90),
  handoffRecallLimit: z.number().int().min(0).max(10),
  handoffTokenBudget: z.number().int().min(0).max(10_000),
...
export const defaultSettings: DekiSettings = settingsSchema.parse({
  ...
  agent: {
    ...
    workerMaxInputTokens: 64_000,
    workerMaxOutputTokens: 16_000,
    workerMaxToolCalls: 50,
    workerModel: "",
    dagExecutionEnabled: false,
    planMaxConcurrentSteps: 2,
    planMaxDurationMs: 1_800_000,
    planMaxInputTokens: 256_000,
    planMaxOutputTokens: 64_000,
    planMaxToolCalls: 200,
    planModelRoutes: {
      coordinator: [],
      explorer: [],
      implementer: [],
