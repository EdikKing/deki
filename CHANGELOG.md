...
- 增加带证据与置信阈值的分层自动项目记忆，以及可过期、可置顶的分支交接摘要。
- 使用 Git common directory 统一项目身份，让主工作区与 linked worktree 共享稳定知识。

### Changed

- 在计划对话中展示重新规划反馈，并扩充记忆中心的治理与召回设置。

## [0.0.10] - 2026-07-30

### Added

- 在当前对话时间线中规划、审批并执行计划任务。
- 为输入区和消息附件增加点击放大的图片查看器。
...
- 修复 macOS 自动更新因未签名发布包不可用的问题，并为自动下载失败提供手动下载入口。
- 修复 Electron 冒烟测试退出卡死、输入框焦点样式冲突及对话布局断言失效问题。
- 修复从任务中心选择侧边栏会话后无法返回聊天界面的问题。
- 为桌面端优雅退出增加超时保护，避免清理过程无限等待。

## [0.0.5] - 2026-07-30

### Fixed

- 修复 Runner 在 Windows 上的工作区路径、Git 换行符测试及包管理器脚本启动兼容性。
