...
| --- | --- | --- |
| `user` | 当前用户 | 长期偏好、通用习惯 |
| `project` | Git common directory | 项目约定、技术背景；主工作区和 linked worktree 共享 |
...
项目会话默认启用分层自动记忆。每次成功对话后，模型最多提取三个稳定知识项，并滚动更新当前分支的会话交接摘要。只有引用用户消息或成功 Tool 结果、且置信度达到默认 `0.85` 阈值的知识会自动生效；助手推断和低置信内容仍作为待确认候选。交接摘要默认保留 14 天，置顶后不会自动过期。系统还会处理语义 key 更新、冲突替代、到期归档和敏感信息过滤。

项目记忆使用 Git canonical common directory 作为身份，因此同一仓库的主目录和 linked worktree 可以共享稳定知识；工作区设置、会话和 `workspace` 记忆仍按具体路径隔离。非 Git 项目自动回退到工作区路径身份，系统不会回填已有会话历史。
